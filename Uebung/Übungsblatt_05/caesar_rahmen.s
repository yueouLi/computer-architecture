...
caesar: ####################################
	...
	add $t4, $t4, $s1   # Add the shift to the current letter
...
save:	####################################
	# Fuegen Sie hier Ihre Loesung ein #
	sb $t4, result($t0) # Save the encrypted letter to result
	addi $t0, $t0, 1    # Increment the position for the next letter
...
cadd:	####################################
	# Fuegen Sie hier Ihre Loesung ein #
	####################################
	sub $t4, $t4, 26   # If the result is larger than 'Z', subtract 26 to loop back to 'A'
